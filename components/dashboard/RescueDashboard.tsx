    ...
    const handleDemoKeys = (e: KeyboardEvent) => {
      ...
      if (e.key.toLowerCase() === "h") {
        ...
        setActiveDetection({
          ...
          status: "VERIFIED",
          contributing_devices: ["ESP32-01", "ESP32-02", "ESP32-03"],
    ...
    const graphInterval = setInterval(() => {
      setTelemetryHistory((prev) => [
...

        {
          ...
          movement: graphMovement + (Math.random() * 1 - 0.5),

          presence: graphPresence + (Math.random() * 1 - 0.5),
        },
      ]);
    }, 500);
  ...
  return (
    <div className="flex flex-col h-screen w-screen bg-slate-950 text-slate-100 overflow-hidden select-none font-mono">
      <TopBar
        ...
        onlineSensors={
          demoRunning ? 3 : devices.filter((d) => d.status === "ONLINE").length
        }
        totalSensors={3}
        survivorProbability={
          activeDetection
        ...
        activeSession={demoRunning || session?.status === "ACTIVE"}
      />
